    ...
    <div class="row">
      ...
      <div class="row mt-3 mb-3 px-5">
        ...
        <button type="button" class="btn btn-primary" @click="editCategory"
                :data-kt-indicator="is_loading? 'on':'off'" :disabled="is_loading">
...
export default {
  ...
  created() {
    this.category = Object.assign({}, this.categoryData)
  },
  data: () => ({
    is_loading: false,
    category: {
  ...
  methods: {
    async editCategory() {
      ...
      await axios.patch(route('dashboard.categories.update', this.categoryData.id), this.category).then((res) => {
