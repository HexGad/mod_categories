    ...
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-title w-75">
        ...
        <div class="d-flex align-items-center position-relative my-1">
          ...
          <!--end::Svg Icon-->
          <input type="text" v-model="datatable.search.value" @input="debounceSearch" :placeholder="placeholder" class="form-control form-control-solid w-250px ps-14"/>
      ...
      <div class="dataTables_wrapper dt-bootstrap4 no-footer">
        <div class="table-responsive">
          <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer">
            <thead>
            <tr>
              <th v-if="$slots.batchActions" class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                <label class="form-check form-check-sm form-check-custom form-check-solid">
                  <input class="form-check-input" type="checkbox" value="">
                </label>
              </th>
              <th @click="orderBy(index, column)" v-for="(column, index) in cols"
            ...
            <tr v-if="rows.length" :class="( index % 2) ? 'odd':'even'" v-for="(row, index) in rows">
              ...
              <td v-for="column in datatable.columns" :class="column.className" v-html="row[column.name]"></td>
              <td> <slot name="actions" :row="row"></slot> </td>
            </tr>
            <tr v-else>
